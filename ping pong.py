import random
import sys

import pygame

# initialization of pygame
pygame.init()
Clock = pygame.time.Clock()
# set screen of window
screen_width = 800
screen_height = 600
screen = pygame.display.set_mode((screen_width, screen_height))
# title og the game
pygame.display.set_caption("PING PONG")

#  ball
ball = pygame.Rect(screen_width / 2 - 15, screen_height / 2 - 15, 10, 10)
#  player
player = pygame.Rect(screen_width - 20, screen_height / 2 - 70, 10, 90)
#  oponent
oponent = pygame.Rect(10, screen_height / 2 - 70, 10, 90)

#  color
bg_color = pygame.Color("light blue")
light_grey = (200, 100, 180)
# ball speed
ball_speed_x = 3 * random.choice((1, -1))
ball_speed_y = 3 * random.choice((1, -1))
# player speed
player_speed = 0
# oponent speed
oponent_speed = 0
# score chat
player_score = 0
oponent_score = 0
game_font = pygame.font.Font("freesansbold.ttf", 32)

# timer
score_time = None


def ball_animation():
    global ball_speed_x, ball_speed_y, player_score, oponent_score, score_time
    ball.x += ball_speed_x
    ball.y += ball_speed_y
    if ball.top <= 0 or ball.bottom >= screen_height:
        ball_speed_y *= -1
    if ball.left <= 0:
        player_score += 1
        score_time = pygame.time.get_ticks()
    if ball.right >= screen_width:
        # ball_speed_x *= -1
        oponent_score += 1
        score_time = pygame.time.get_ticks()
        # collision of ball in player and oponent
    if ball.colliderect(player) or ball.colliderect(oponent):
        ball_speed_x *= -1


def ball_restart():
    global ball_speed_x, ball_speed_y, cureent_time, score_time
    cureent_time = pygame.time.get_ticks()
    ball.center = (screen_width / 2, screen_height / 2)
    if cureent_time - score_time < 700:
        num_three = game_font.render("3", False, light_grey)
        screen.blit(num_three, (screen_width / 2 - 10, screen_height / 2 - 20))
    if 700 < cureent_time - score_time < 1400:
        num_num = game_font.render("2", False, light_grey)
        screen.blit(num_num, (screen_width / 2 - 10, screen_height / 2 - 20))
    if 1400 < cureent_time - score_time < 2100:
        num_one = game_font.render("1", False, light_grey)
        screen.blit(num_one, (screen_width / 2 - 10, screen_height / 2 - 20))

    if cureent_time - score_time < 2000:
        ball_speed_x, ball_speed_y = 0, 0
    else:
        ball_speed_y = 3 * random.choice((1, -1))
        ball_speed_x = 3 * random.choice((1, -1))
        score_time = None
    # ball_speed_y *= random.choice((1, -1))
    # ball_speed_x *= random.choice((1, -1))


def player_animation():
    player.y += player_speed
    if player.top <= 0:
        player.top = 0
    if player.bottom >= screen_height:
        player.bottom = screen_height


def oponent_animation():
    oponent.y += oponent_speed
    if oponent.top <= 0:
        oponent.top = 0
    if oponent.bottom >= screen_height:
        oponent.bottom = screen_height


while True:

    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()

        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_DOWN:
                player_speed += 7
            if event.key == pygame.K_UP:
                player_speed -= 7
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_DOWN:
                player_speed -= 7
            if event.key == pygame.K_UP:
                player_speed += 7
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_s:
                oponent_speed += 7
            if event.key == pygame.K_w:
                oponent_speed -= 7
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_s:
                oponent_speed -= 7
            if event.key == pygame.K_w:
                oponent_speed += 7

    ball_animation()
    player_animation()
    oponent_animation()
    # visulas
    screen.fill(bg_color)
    pygame.draw.rect(screen, light_grey, player)
    pygame.draw.rect(screen, light_grey, oponent)
    pygame.draw.rect(screen, light_grey, ball)
   # pygame.draw.aaline(screen, light_grey, (screen_width / 2, 0), (screen_width / 2, screen_height))
    if score_time:
        ball_restart()
    player_text = game_font.render(f"player: {player_score}", False, light_grey)
    screen.blit(player_text, (10, 10))
    oponent_text = game_font.render(f"opponent:{oponent_score}", False, light_grey)
    screen.blit(oponent_text, (560, 10))

    pygame.display.flip()
    Clock.tick(60)
